...
function updateThresholdBounds(rows, clampCurrent = true) {
  ...
  const toeicCheck = rows.map((row) => row.toeic).filter((v) => Number.isFinite(v));
  if (!toeicCheck.length) return;
  ...
  const minToeic = Math.min(...toeicValues);
  const maxToeic = Math.max(...toeicValues);
...
function getAnalysisRows() {
  ...
  rows.forEach((row) => {
    ...
    if (!previous || row.toeic > previous.toeic || (row.toeic === previous.toeic && row.gpa > previous.gpa)) {
...
function findTargetCandidates(target) {
  ...
  const toeicValues = state.rows.map((row) => row.toeic);
